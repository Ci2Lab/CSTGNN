...
def evaluate(data, X, Y, model, evaluateL2, evaluateL1, batch_size):
    ...
    for X, Y in data.get_batches(X, Y, batch_size, False):
        ...
        metrics = metric(output * scale, Y * scale)
...
parser.add_argument('--graph_data', type=str, default='adj.npz',
                    help='location of the adjacency file')
...
parser.add_argument('--num_nodes',type=int,default=62,help='number of nodes/variables')
...
parser.add_argument('--seq_in_len',type=int,default=60,help='input sequence length')
...
adj = sp.load_npz(args.graph_data)
adj = adj.todense()
#convert adj matrix to edge index sparse
# adj = np.ones_like(adj)-np.eye(len(adj))
edges = []
...
#convert adj matrix and edge index to torch tensor
    ...
    try:
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            val_loss, val_rae, val_corr, val_Mae, val_Mse, val_Mape = evaluate(Data, Data.valid[0], Data.valid[1], model, evaluateL2, evaluateL1,
                                               args.batch_size)
            print(
                '| end of epoch {:3d} | time: {:5.2f}s | train_loss {:5.4f} | valid rse {:5.4f} | valid rae {:5.4f} | valid corr  {:5.4f} | valid mae  {:5.4f} | valid rmse  {:5.4f} | valid mape  {:5.4f} '.format(
